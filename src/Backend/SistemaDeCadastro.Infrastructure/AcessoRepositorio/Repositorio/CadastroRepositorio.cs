...
public class CadastroRepositorio : ICadastroReadOnlyRepositorio, ICadastroWriteOnlyRepositorio, ICadastroUpdateOnlyRepositorio
{
    ...
    async Task<Cadastro> ICadastroReadOnlyRepositorio.RecuperarPorId(long cadastroId) => await _contexto.Cadastros.AsNoTracking().FirstOrDefaultAsync(cadastro => cadastro.Id == cadastroId);
    ...
    async Task<Cadastro> ICadastroUpdateOnlyRepositorio.RecuperarPorId(long cadastroId) => await _contexto.Cadastros.AsNoTracking().FirstOrDefaultAsync(cadastro => cadastro.Id == cadastroId);
}